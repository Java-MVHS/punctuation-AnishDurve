...
// Date: 12/11
// Punctuation.java
// Working on: removing words from a set of words 
/*
 * Pseudocode:
 * class header
 * default constructor
 ...
 * 	calls getPunctuation(str)
 * 
 * getPunctuation(String text)
 * 	have for loop that iterates through whole string
 * 	if a space is read call checkForPunctuation(pass in the word using substring)
 * 	if there is punctuation, add the word to the text to be printed in a variable
 *  after for loop, call printWords(send in altered text)
 * 
 * checkForPunctuation(String word)
 * 	have for loop that iterates through the word
 * 	if punctuation is present at the index, return a true for punctuation
 * 	else return false for punctuation
 * 
 * printWords(String printedIn)
...
public class Punctuation
{
	...
	{
		Punctuation p = new Punctuation();	// creates instance of class
		p.searchIt();						// calls searchIt()
	}
	
	public void searchIt()					// calls getPunctuation();
	{
		System.out.println("\n\n\n");
		...
		getPunctuationWords(str);	// calls getPunctuation and passes the string
	}
	...
	public void getPunctuationWords(String text)
	{
		...
		for (int count = 1; count < text.length(); count++) // this loop continues until it has gone through the entire string
		{
			if (text.charAt(count) == ' ')					// checks when there is a new word by checking for a new space
			{		
				punctuation = checkForPunctuation(text.substring(count2,count));	// calls checkForPunctuation() and passes in the word
				if (punctuation == true)					// if there is punctuation then the word is added to the string to be printed
				{
					printed = printed + text.substring(count2,count);		// adds the word to the text to be printed
					punctuation = false;
				}
				count2 = count;
			}
		}
		printWords(printed);								// calls printWords to print the modified text
	}
	...
	public boolean checkForPunctuation(String word)
	{
		...
		for (int i = 0; i < word.length(); i++)				// iterates through the word
		{
			...
			// this if statement checks for any punctuation
			{
				hasPunctuation = true;
			}
		}
		return hasPunctuation;					// returns boolean
		
	}
	
	public void printWords(String printedIn)		// method to print text
	{
		System.out.println(printedIn);
		System.out.println("\n\n\n");
